...
class Model{
    ...
    public function excute($options = []){
        try{
    ...
    public function excuteQuery($options = [], $fetchMode = self::FETCH_ALL){
        // Mặc định là lấy nhiều bản ghi
        $result = $this->excute($options);
        if(!$result){
            return false;
        }
    ...
    public function all($options = []){
        return $this->excuteQuery($options, self::FETCH_ALL);
    }
    public function first($options = []){
        return $this->excuteQuery($options, self::FETCH_FIRST);
    }
    // Đóng kết nối
